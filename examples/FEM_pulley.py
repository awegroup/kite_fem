...
#Apply external forces at bottom node
fe = np.zeros(Pulleys.N)
fe[(Pulleys.num_nodes-1)*6+1] = -100
fe[(Pulleys.num_nodes-1)*6] = 50

#Plot, solve and then plot again
ax1,fig1 = plot_structure(Pulleys,fe=fe,plot_external_forces=True,fe_magnitude=0.35,plot_2d=True)
Pulleys.solve(fe = fe, tolerance=1e-3, convergence_criteria = "residual", max_iterations=5000, step_limit=0.3, relax_init=0.5,relax_update=0.95, k_update=1,I_stiffness=15)
...
#Verification check, for an ideal frictionless pulley, angles should be equal
